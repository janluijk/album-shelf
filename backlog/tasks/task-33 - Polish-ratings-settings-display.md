---
id: TASK-33
title: Polish ratings settings display
status: To Do
assignee: []
created_date: '2026-07-07 08:40'
labels:
  - ui-polish
dependencies: []
priority: medium
ordinal: 32000
---

## Description

<!-- SECTION:DESCRIPTION:BEGIN -->
The ratings granularity options in the settings page currently display as plain bullet points, which feels inconsistent with the app's polished design. Replace the bullet-point list with a more visually appealing presentation that better integrates with the app's design system.

The functionality remains unchanged — users select between integer, half-star, and decimal rating granularities. Only the visual presentation should be improved.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Ratings granularity options display with custom styling instead of default bullet points
- [ ] #2 All three granularity options (integer, half-star, decimal) remain selectable and functional
- [ ] #3 Selected option is visually distinguished from unselected options
- [ ] #4 Visual styling is consistent with the app's design tokens (colors, spacing, typography, borders, rounded corners)
- [ ] #5 Display is responsive and looks good on mobile, tablet, and desktop viewports
<!-- AC:END -->
